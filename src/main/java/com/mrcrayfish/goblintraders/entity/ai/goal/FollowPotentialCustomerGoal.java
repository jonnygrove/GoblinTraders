...
public class FollowPotentialCustomerGoal extends Goal
{
    ...
    public void tick()
    {
        ...
        else if(!this.hitOnce)
        {
            if(this.hitTimer-- == 0)
            {
                this.entity.world.playSound(null, this.entity.getPosition().getX(), this.entity.getPosition().getY(), this.entity.getPosition().getZ(), ModSounds.ENTITY_GOBLIN_TRADER_ANNOYED_GRUNT, SoundCategory.NEUTRAL, 1.0F, 0.9F + this.entity.getRNG().nextFloat() * 0.2F);
